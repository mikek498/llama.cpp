...
void ggml_vec_abs_f32(const int n, float * y, const float * x) {
    int i = 0;
#if defined(__AVX512F__)
    // AVX512F has a specific instruction for absolute value of packed single-precision floats
    for (; i + 15 < n; i += 16) {
        _mm512_storeu_ps(y + i, _mm512_abs_ps(_mm512_loadu_ps(x + i)));
    }
#elif defined(__AVX2__) && defined(__FMA__) // AVX2 implies FMA for our typical use cases
    // For AVX2, clear the sign bit (MSB) using a bitwise AND.
    // 0x7FFFFFFF is a mask where all bits are 1 except the sign bit.
    const __m256 mask = _mm256_castsi256_ps(_mm256_set1_epi32(0x7FFFFFFF));
    for (; i + 7 < n; i += 8) {
        _mm256_storeu_ps(y + i, _mm256_and_ps(_mm256_loadu_ps(x + i), mask));
    }
#elif defined(__SSE2__)
    // For SSE2, similar to AVX2, clear the sign bit using a bitwise AND.
    const __m128 mask = _mm_castsi128_ps(_mm_set1_epi32(0x7FFFFFFF));
    for (; i + 3 < n; i += 4) {
        _mm_storeu_ps(y + i, _mm_and_ps(_mm_loadu_ps(x + i), mask));
    }
#elif defined(__ARM_NEON) && defined(__aarch64__)
    // NEON has a specific instruction for absolute value.
    for (; i + 3 < n; i += 4) {
...
void ggml_vec_hardsigmoid_f32(const int n, float * y, const float * x) {
    ...
    for (; i + 15 < n; i += 16) {
        __m512 vx = _mm512_loadu_ps(x + i);
        vx = _mm512_add_ps(vx, v_three);
        vx = _mm512_mul_ps(vx, v_six_inv);
        vx = _mm512_max_ps(v_zero, vx);
        vx = _mm512_min_ps(v_one, vx);
        _mm512_storeu_ps(y + i, vx);
    }
#elif defined(__AVX2__) && defined(__FMA__)
    const __m256 v_zero = _mm256_set1_ps(0.0f);
    const __m256 v_one = _mm256_set1_ps(1.0f);
    ...
    for (; i + 7 < n; i += 8) {
        __m256 vx = _mm256_loadu_ps(x + i);
        vx = _mm256_add_ps(vx, v_three);
        vx = _mm256_mul_ps(vx, v_six_inv);
        vx = _mm256_max_ps(v_zero, vx);
        vx = _mm256_min_ps(v_one, vx);
    ...
    for (; i + 3 < n; i += 4) {
        __m128 vx = _mm_loadu_ps(x + i);
        vx = _mm_add_ps(vx, v_three);
        vx = _mm_mul_ps(vx, v_six_inv);
        vx = _mm_max_ps(v_zero, vx);
        vx = _mm_min_ps(v_one, vx);
        _mm_storeu_ps(y + i, vx);
    }
#elif defined(__ARM_NEON) && defined(__aarch64__)
    const float32x4_t v_zero = vdupq_n_f32(0.0f);
    const float32x4_t v_one = vdupq_n_f32(1.0f);
    ...
    for (; i + 3 < n; i += 4) {
        float32x4_t vx = vld1q_f32(x + i);
        vx = vaddq_f32(vx, v_three);
        vx = vmulq_f32(vx, v_six_inv);
        vx = vmaxq_f32(v_zero, vx);
        vx = vminq_f32(v_one, vx);
        vst1q_f32(y + i, vx);
    }
#endif
    // Scalar loop for leftovers or if no SIMD
